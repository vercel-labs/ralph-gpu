...
// Re-export UniformValue for convenience
export type { UniformValue, UniformObject, UniformRawValue, RenderTarget };
...
export interface PassOptions {
  ...
  vertexShader?: string;
  /** Initial uniforms with { value: X } pattern */
  uniforms?: Record<string, UniformObject>;
}
 ...
 * Automatically injects custom uniform declarations based on provided uniforms.
 * Supports Three.js-style { value: X } uniform pattern.
 */
export class Pass {
  ...
  /** Custom uniform values with { value: X } pattern */
  uniforms: Record<string, UniformObject> = {};
  
  /** Original fragment shader source (for recompilation if needed) */
  ...
  constructor(
    ...
    // Initialize uniforms from options first (needed for injection)
    if (options.uniforms) {
      this.uniforms = { ...options.uniforms };
    }
    
    // Process shader: inject global and custom uniform declarations
    let processedFragmentShader = injectGlobalUniforms(fragmentShader);
    processedFragmentShader = injectCustomUniforms(processedFragmentShader, this.uniforms);
    
    // Compile shaders and create program
    this.program = this.createProgram(gl, vertexShader, processedFragmentShader);
  ...
  /**
   * Set a uniform value (Three.js style)
   * @param name - Uniform name
   * @param value - Uniform value (raw or wrapped in { value: X })
   * @returns The Pass instance for chaining
   */
  setUniform(name: string, value: UniformValue): this {
    // Wrap in { value: X } if not already wrapped
    if (value !== null && typeof value === 'object' && 'value' in value) {
      this.uniforms[name] = value as UniformObject;
    } else {
      this.uniforms[name] = { value: value as UniformRawValue };
    }
    return this;
  }
  ...
  private applyUniforms(): void {
    ...
    // Track texture units for sampler2D uniforms
    let textureUnit = 0;
    ...
    // Apply custom uniforms using the setUniform function
    for (const [name, uniformObj] of Object.entries(this.uniforms)) {
      const location = this.getUniformLocation(name);
      if (!location) continue;
      
      const usedUnit = setUniformValue(gl, location, uniformObj, textureUnit);
      if (usedUnit >= 0) {
        textureUnit = usedUnit + 1;
      }
    }
  }
