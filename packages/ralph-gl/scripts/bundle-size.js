...
/**
 * Bundle Size Reporter for ralph-gl
 * 
...
const TARGET_SIZE_KB = 10; // Target: < 10kB gzipped
...
function printTable(data) {
  ...
  console.log(`\n${colors.bold}${colors.cyan}📦 ralph-gl Bundle Size Report${colors.reset}\n`);
  ...
  for (const row of data) {
    ...
    const gzipColor = gzipKb > TARGET_SIZE_KB ? colors.red : colors.green;
    ...
    console.log(
      ...
      `${colors.dim}|${colors.reset} ` +
      `${gzipColor}${formatBytes(row.gzip).padStart(colWidths.gzip - 2)}${colors.reset}` +
      `${colors.dim}|${colors.reset} ` +
  ...
  // Check target
  const esmSize = data.find(s => s.file === 'index.mjs');
  if (esmSize) {
    const gzipKb = esmSize.gzip / 1024;
    if (gzipKb > TARGET_SIZE_KB) {
      console.log(`\n${colors.red}⚠ Bundle exceeds target of ${TARGET_SIZE_KB}kB gzipped!${colors.reset}`);
    } else {
      console.log(`\n${colors.green}✓ Bundle is within target (< ${TARGET_SIZE_KB}kB gzipped)${colors.reset}`);
    }
  }
}
...
> 📦 **~${gzipKb} kB** gzipped (ESM) - Target: < ${TARGET_SIZE_KB}kB`;
